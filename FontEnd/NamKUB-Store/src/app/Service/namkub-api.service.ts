...
export class NAMKUBAPIService {
  ...
  private stocks = [...MOCK_STOCKS];
  private restocks = [...MOCK_RESTOCKS];
  private users = this.loadUsers();
  private orders = [...MOCK_ORDERS];
  private summaries = [...MOCK_SUMMARIES];
  private alltime = [...MOCK_ALLTIME];
  private bestSale = [...MOCK_BEST_SALE];
  ...
  getActiveProducts(): Observable<Products[]>{
    return of(this.products.filter(product => product.Product_status === 'active'));
  }
  ...
  addProduct(product: Partial<Products> & { StockQuantity?: number; SupUnitPrice?: number }): Observable<Products> {
    const newProduct: Products = {
      ...
      Product_Name: product.Product_Name || 'Mock Product',
    ...
    this.stocks = [...this.stocks, {
      ...
      Sup_Unitprice: 0,
      Sup_Name: this.suppliers.find(supplier => String(supplier.Sup_ID) === String(newProduct.Sup_ID))?.Sup_Name || 'Mock Supplier'
    }];
  ...
  addRestock(restockData: Pick<Restock, 'Restock_Quantity' | 'Restock_Unitprice' | 'Stock_ID'>): Observable<Restock> {
    ...
    const newRestock: Restock = {
      ...
      Product_Name: stock?.Product_Name || 'Mock Product',
  ...
  login(username: string, password: string): Observable<{ token: string }> {
    ...
    return of({ token: this.createMockJwt(user) });
  }
  ...
  private createMockJwt(user: Users): string {
